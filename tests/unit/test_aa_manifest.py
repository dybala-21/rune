...
def test_bench_run_dry_run_writes_attempt_artifacts(tmp_path):
    ...
    assert agent_config["benchmark_prompt_policy"] == "aa-coding-agent-v8"
    ...
    assert fingerprint["benchmark_prompt_policy"] == "aa-coding-agent-v8"
    ...
    assert "constrained editing or macro/script-generation tasks" in effective_instruction
    assert "Smoke checks must fail the command" in effective_instruction
    assert "Do not schedule multiple alternative artifact writes" in effective_instruction
...
def test_fingerprint_gate_checks_source_diff(monkeypatch):
    ...
    gate = evaluate_fingerprint_gate(
        {
            "benchmark_prompt_policy": "aa-coding-agent-v7",
...
def test_build_agent_instruction_preserves_task_instruction(tmp_path):
    ...
    assert "exact-output transformation tasks" in instruction
    assert "large Vim macro or text-transformation tasks" in instruction
    assert "the string is parsed twice" in instruction
    assert "Pattern not found" in instruction
    assert "Stay inside the task's allowed-syntax list" in instruction
    assert "Shape reference for a setreg-based substitution macro" in instruction
    assert "Do not increase benchmark smoke-test timeouts" in instruction
    assert "first mismatching public row" in instruction
