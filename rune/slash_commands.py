"""Slash command registry shared by every chat surface.

Defines the built-in slash commands (/help, /goal, /diff, ...) and the parser
that detects them in user input. The TUI dispatches the __ACTION__ markers via
RuneApp; the web/API server executes them via rune.api.command_actions.
"""
...
async def _memory_handler(args: str) -> str | None:
    ...
    key = parts[0].lower()
    if key not in ("show", "add", "clear"):
        return "Usage: /memory [show|add <text>|clear]"
    if key == "add" and len(parts) > 1:
        # Inline text (web/API path); bare "add" stays interactive in the TUI.
        return f"__ACTION__:memory:add:{parts[1]}"
    return f"__ACTION__:memory:{key}"
